...
export const downloadBmc = async (id: number) => {
  try {
    ...
    const headerWrapper = wrapper.querySelector('[class*="BmcCanvasHeaderWrapper"]') as HTMLElement;
    if (headerWrapper) {
    ...
    const bmcCanvas = wrapper.querySelector('[class*="BmcCanvas"]') as HTMLElement;
    if (bmcCanvas) {
...
export const downloadCompetitorAnalysis = async (id: number, title: string) => {
  try {
    ...
    const analysisData = analysesResponse.data.find(item => item.bmcId === id);
    ...
    try {
      ...
      if (totalPages === 1) {
        ...
        for (let i = 0; i < totalPages; i++) {
          ...
          // 현재 페이지에서 그릴 높이 계산
          const pageHeight = Math.min(pageContentHeight, scaledHeight - position);
